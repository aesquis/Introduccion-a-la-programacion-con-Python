#La meva proposta
countryCo = input("Des de quin pais realitzararas la compra? ").upper()
totalCompra = input("Quin es l'import total de la teva compra? ")
if countryCo == "CANADA":
    provincia = input("Des de quina provincia realites la teva compra? ").upper()

    if countryCo == "CANADA" and (provincia == "ALBERTA"):
        totalFactura = float(totalCompra) * 1.05
        print ("El seu total de factura es de: %d $"  % totalFactura)

    elif countryCo == "CANADA" and (provincia == "ONTARIO" or provincia == "NEW BRUNSWICK" or provincia == "NOVA SCOTIA"):
        totalFactura = float(totalCompra) * 1.13
        print ("El seu total de factura es de: %d $"  % totalFactura)

    else:
        totalFactura = float(totalCompra) * 1.011
        print ("El seu total de factura es de: %d $"  % totalFactura)

else:
    print("Moltes gracies per a realitzar la seva compra")
